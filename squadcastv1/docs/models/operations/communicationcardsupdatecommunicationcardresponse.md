# CommunicationCardsUpdateCommunicationCardResponse

The request has succeeded.

## Example Usage

```typescript
import { CommunicationCardsUpdateCommunicationCardResponse } from "SquadcastSDK/models/operations";

let value: CommunicationCardsUpdateCommunicationCardResponse = {
  data: {
    createdAt: "1708485645936",
    updatedAt: "1735622533154",
    deletedAt: "<value>",
    id: 575441,
    incidentId: "<id>",
    url: "https://worthy-underneath.biz/",
    type: "<value>",
    title: "<value>",
    channelId: "<id>",
    archivedAt: "<value>",
  },
};
```

## Fields

| Field                                                                                                                                   | Type                                                                                                                                    | Required                                                                                                                                | Description                                                                                                                             |
| --------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `data`                                                                                                                                  | [models.V3IncidentsCommunicationCardsCommunicationCardResponse](../../models/v3incidentscommunicationcardscommunicationcardresponse.md) | :heavy_check_mark:                                                                                                                      | N/A                                                                                                                                     |