...
* [get](#get) - Get details of Audit Logs export history by ID
...
async function run() {
  const result = await squadcastSDK.auditLogs.exportHistory.get({
    id: "<id>",
  });
...
import { auditLogsExportHistoryGet } from "@solarwinds/squadcast-sdk-typescript/funcs/auditLogsExportHistoryGet.js";
...
async function run() {
  const res = await auditLogsExportHistoryGet(squadcastSDK, {
    id: "<id>",
  });
  if (res.ok) {
    ...
    console.log("auditLogsExportHistoryGet failed:", res.error);
  }
}
