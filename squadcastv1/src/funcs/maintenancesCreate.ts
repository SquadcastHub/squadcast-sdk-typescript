 ...
 * Create Maintenance
 */
export function maintenancesCreate(
  client: SquadcastSDKCore,
  request: operations.MaintenancesCreateMaintenanceRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.MaintenancesCreateMaintenanceResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.MaintenancesCreateMaintenanceRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON(
    "body",
    payload["V4.StatusPages.Maintenances.CreateMaintenanceRequest"],
    { explode: true },
  );
  ...
  const path = pathToFunc("/v4/statuspages/{statuspageID}/maintenance")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.MaintenancesCreateMaintenanceResponse$inboundSchema),
