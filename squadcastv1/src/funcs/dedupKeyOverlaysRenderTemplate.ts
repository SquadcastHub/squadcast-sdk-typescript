 ...
 */
export function dedupKeyOverlaysRenderTemplate(
  client: SquadcastSDKCore,
  request: operations.OverlayRenderDedupKeyTemplateRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.OverlayRenderDedupKeyTemplateResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.OverlayRenderDedupKeyTemplateRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON(
    "body",
    payload["V3.Services.Overlay.RenderDedupKeyTemplateRequest"],
    { explode: true },
  );
  ...
  const path = pathToFunc("/v3/services/{serviceID}/overlays/dedup-key/render")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.OverlayRenderDedupKeyTemplateResponse$inboundSchema),
