 ...
 */
export function statusPagesComponentsUpdateById(
  client: SquadcastSDKCore,
  request: operations.ComponentsUpdateComponentByIdRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.ComponentsUpdateComponentByIdResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ComponentsUpdateComponentByIdRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON(
    "body",
    payload["V4.StatusPages.Components.UpdateComponentByIdRequest"],
    { explode: true },
  );

  const pathParams = {
    component_id: encodeSimple("component_id", payload.component_id, {
      explode: false,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ComponentsUpdateComponentByIdResponse$inboundSchema),
