 ...
 */
export function incidentsUpdatePriority(
  client: SquadcastSDKCore,
  request: operations.IncidentsIncidentPriorityUpdateRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.IncidentsIncidentPriorityUpdateResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.IncidentsIncidentPriorityUpdateRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON(
    "body",
    payload["V3.Incidents.IncidentPriorityUpdateRequest"],
    { explode: true },
  );
  ...
  const path = pathToFunc("/v3/incidents/{incidentID}/priority")(pathParams);
  ...
  >(
    M.json(
      200,
      operations.IncidentsIncidentPriorityUpdateResponse$inboundSchema,
    ),
