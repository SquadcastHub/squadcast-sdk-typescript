 ...
 * Create Notes
 */
export function incidentsNotesCreate(
  client: SquadcastSDKCore,
  request: operations.NotesCreateNotesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.NotesCreateNotesResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.NotesCreateNotesRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON(
    "body",
    payload["V3.Incidents.Notes.CreateNoteRequest"],
    { explode: true },
  );
  ...
  const path = pathToFunc("/v3/incidents/{IncidentId}/warroom")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.NotesCreateNotesResponse$inboundSchema),
