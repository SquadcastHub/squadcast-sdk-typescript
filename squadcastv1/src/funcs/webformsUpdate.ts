...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.WebformsUpdateWebformRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON(
    "body",
    payload["V3.Webforms.CreateOrUpdateWebformRequest"],
    { explode: true },
  );

  const pathParams = {
    webformId: encodeSimple("webformId", payload.webformId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/v3/webform/{webformId}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.WebformsUpdateWebformResponse$inboundSchema),
