 ...
 * Update Issue
 */
export function statusPagesIssuesUpdate(
  client: SquadcastSDKCore,
  request: operations.IssuesUpdateIssueRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.IssuesUpdateIssueResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.IssuesUpdateIssueRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON(
    "body",
    payload["V4.StatusPages.Issues.UpdateIssueRequest"],
    { explode: true },
  );
  ...
  const path = pathToFunc("/v4/statuspages/{statuspageID}/issues/{issue_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.IssuesUpdateIssueResponse$inboundSchema),
