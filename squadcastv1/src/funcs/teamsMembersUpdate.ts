 ...
 */
export function teamsMembersUpdate(
  client: SquadcastSDKCore,
  request: operations.TeamsUpdateTeamMemberRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.TeamsUpdateTeamMemberResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.TeamsUpdateTeamMemberRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["V3.Teams.UpdateTeamMemberRequest"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/v3/teams/{teamId}/members/{memberId}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.TeamsUpdateTeamMemberResponse$inboundSchema),
