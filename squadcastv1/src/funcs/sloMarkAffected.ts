...
> {
  const parsed = safeParse(
    request,
    (value) => operations.SLOMarkSLOAffectedRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["V3.SLO.MarkSLOAffectedRequest"], {
    explode: true,
  });

  const pathParams = {
    sloID: encodeSimple("sloID", payload.sloID, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/v3/slo/{sloID}/incident")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.SLOMarkSLOAffectedResponse$inboundSchema),
