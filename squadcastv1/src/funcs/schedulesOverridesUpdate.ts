 ...
 */
export function schedulesOverridesUpdate(
  client: SquadcastSDKCore,
  request: operations.OverridesUpdateScheduleOverrideRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.OverridesUpdateScheduleOverrideResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.OverridesUpdateScheduleOverrideRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload["V4.UpdateScheduleOverrideRequest"], {
    explode: true,
  });

  const pathParams = {
    overrideID: encodeSimple("overrideID", payload.overrideID, {
      explode: false,
  ...
  >(
    M.json(
      200,
      operations.OverridesUpdateScheduleOverrideResponse$inboundSchema,
    ),
    M.jsonErr(
