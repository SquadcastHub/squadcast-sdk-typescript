...
export function schedulesUpdate(
  ...
  Result<
    operations.SchedulesUpdateScheduleResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.SchedulesUpdateScheduleRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["V4.UpdateScheduleRequest"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/v4/schedules/{scheduleID}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.SchedulesUpdateScheduleResponse$inboundSchema),
    M.jsonErr(
