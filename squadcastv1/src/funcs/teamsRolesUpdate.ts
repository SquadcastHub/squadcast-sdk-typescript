...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.TeamsUpdateTeamRoleRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["V3.Teams.UpdateTeamRoleRequest"], {
    explode: true,
  });

  const pathParams = {
    roleId: encodeSimple("roleId", payload.roleId, {
      explode: false,
  ...
  const path = pathToFunc("/v3/teams/{teamId}/roles/{roleId}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.TeamsUpdateTeamRoleResponse$inboundSchema),
