 ...
 */
export function globalEventRulesRulesetsCreate(
  client: SquadcastSDKCore,
  request: operations.GlobalEventRulesCreateRulesetRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GlobalEventRulesCreateRulesetResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GlobalEventRulesCreateRulesetRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON(
    "body",
    payload["V3.GlobalEventRules.CreateRulesetRequest"],
    { explode: true },
  );
  ...
  const path = pathToFunc("/v3/global-event-rules/{ger_id}/rulesets")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.GlobalEventRulesCreateRulesetResponse$inboundSchema),
