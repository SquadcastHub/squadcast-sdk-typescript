 ...
 */
export function servicesAptaConfigCreateOrUpdate(
  client: SquadcastSDKCore,
  request: operations.ServicesCreateOrUpdateAPTAConfigRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.ServicesCreateOrUpdateAPTAConfigResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ServicesCreateOrUpdateAPTAConfigRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload["V3.Services.APTAConfigRequest"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/v3/services/{serviceID}/apta-config")(pathParams);
  ...
  >(
    M.json(
      200,
      operations.ServicesCreateOrUpdateAPTAConfigResponse$inboundSchema,
    ),
