 ...
 * Attach Runbooks
 */
export function incidentsAttachRunbooks(
  client: SquadcastSDKCore,
  request: operations.RunbooksAttachRunbooksRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.RunbooksAttachRunbooksResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.RunbooksAttachRunbooksRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON(
    "body",
    payload["V3.Incidents.Runbooks.AttachRunbooksRequest"],
    { explode: true },
  );
  ...
  const path = pathToFunc("/v3/incidents/{IncidentId}/runbooks")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.RunbooksAttachRunbooksResponse$inboundSchema),
