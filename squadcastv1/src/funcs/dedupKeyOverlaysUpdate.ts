 ...
 */
export function dedupKeyOverlaysUpdate(
  client: SquadcastSDKCore,
  request: operations.OverlayUpdateDedupKeyOverlayRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.OverlayUpdateDedupKeyOverlayResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.OverlayUpdateDedupKeyOverlayRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON(
    "body",
    payload["V3.Services.Overlay.UpdateDedupKeyOverlayRequest"],
    { explode: true },
  );

  const pathParams = {
    alertSource: encodeSimple("alertSource", payload.alertSource, {
      explode: false,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.OverlayUpdateDedupKeyOverlayResponse$inboundSchema),
