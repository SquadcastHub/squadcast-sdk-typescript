 ...
 */
export function schedulesActionsPauseResume(
  client: SquadcastSDKCore,
  request: operations.SchedulesPauseresumeScheduleRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.SchedulesPauseresumeScheduleResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.SchedulesPauseresumeScheduleRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/v4/schedules/{scheduleID}/actions")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.SchedulesPauseresumeScheduleResponse$inboundSchema),
    M.jsonErr(
