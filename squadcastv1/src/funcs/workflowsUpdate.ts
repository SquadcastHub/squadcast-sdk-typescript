...
export function workflowsUpdate(
  ...
  Result<
    operations.WorkflowsUpdateWorkflowResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.WorkflowsUpdateWorkflowRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON(
    "body",
    payload["V3.Workflows.CreateWorkflowRequestUpdate"],
    { explode: true },
  );
  ...
  const path = pathToFunc("/v3/workflows/{workflowID}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.WorkflowsUpdateWorkflowResponse$inboundSchema),
