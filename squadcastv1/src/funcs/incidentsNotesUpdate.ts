 ...
 */
export function incidentsNotesUpdate(
  client: SquadcastSDKCore,
  request: operations.NotesUpdateNoteRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.NotesUpdateNoteResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.NotesUpdateNoteRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON(
    "body",
    payload["V3.Incidents.Notes.UpdateNoteRequest"],
    { explode: true },
  );
  ...
  const pathParams = {
    ...
    NoteId: encodeSimple("NoteId", payload.NoteId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/v3/incidents/{IncidentId}/warroom/{NoteId}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.NotesUpdateNoteResponse$inboundSchema),
