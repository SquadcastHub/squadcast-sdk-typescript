...
export function schedulesList(
  ...
  Result<
    operations.SchedulesListSchedulesResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.SchedulesListSchedulesRequest$outboundSchema.parse(value),
  ...
  const query = encodeFormQuery({
    ...
    "withoutEscalationPolicy": payload.withoutEscalationPolicy,
    "youAndYourSquads": payload.youAndYourSquads,
  }, { explode: false });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.SchedulesListSchedulesResponse$inboundSchema),
    M.jsonErr(
