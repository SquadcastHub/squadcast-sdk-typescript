 ...
 */
export function statusPagesMaintenancesUpdateById(
  client: SquadcastSDKCore,
  request: operations.MaintenancesUpdateMaintenanceByIdRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.MaintenancesUpdateMaintenanceByIdResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.MaintenancesUpdateMaintenanceByIdRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON(
    "body",
    payload["V4.StatusPages.Maintenances.UpdateMaintenanceByIdRequest"],
    { explode: true },
  );

  const pathParams = {
    maintenance_id: encodeSimple("maintenance_id", payload.maintenance_id, {
      explode: false,
  ...
  >(
    M.json(
      200,
      operations.MaintenancesUpdateMaintenanceByIdResponse$inboundSchema,
    ),
