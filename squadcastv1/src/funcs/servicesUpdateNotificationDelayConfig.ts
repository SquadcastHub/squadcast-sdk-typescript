 ...
 */
export function servicesUpdateNotificationDelayConfig(
  client: SquadcastSDKCore,
  request: operations.ServicesDelayedNotificationConfigRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.ServicesDelayedNotificationConfigResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ServicesDelayedNotificationConfigRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON(
    "body",
    payload["V3.Services.NotificationDelayConfigRequest"],
    { explode: true },
  );
  ...
  const path = pathToFunc("/v3/services/{serviceID}/notification-delay-config")(
    pathParams,
  );
  ...
  >(
    M.json(
      200,
      operations.ServicesDelayedNotificationConfigResponse$inboundSchema,
    ),
