 ...
 * Change Timezone
 */
export function schedulesChangeTimezone(
  client: SquadcastSDKCore,
  request: operations.SchedulesChangeTimezoneRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.SchedulesChangeTimezoneResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.SchedulesChangeTimezoneRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v4/schedules/{scheduleID}/change-timezone")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.SchedulesChangeTimezoneResponse$inboundSchema),
    M.jsonErr(
