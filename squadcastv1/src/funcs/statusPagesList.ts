...
export function statusPagesList(
  ...
  Result<
    models.V4StatusPagesListStatusPagesResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.StatusPagesListStatusPagesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v4/statuspages")();

  const query = encodeFormQuery({
    "filters.isPublic": payload["filters.isPublic"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.V4StatusPagesListStatusPagesResponse$inboundSchema),
