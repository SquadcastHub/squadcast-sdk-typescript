 ...
 * Update Tag
 */
export function incidentsTagsUpdate(
  client: SquadcastSDKCore,
  request: operations.TagsUpdateTagRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.TagsUpdateTagResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.TagsUpdateTagRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON(
    "body",
    payload["V3.Incidents.Tags.UpdateTagRequest"],
    { explode: true },
  );
  ...
  const path = pathToFunc("/v3/incidents/{IncidentId}/tags")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.TagsUpdateTagResponse$inboundSchema),
