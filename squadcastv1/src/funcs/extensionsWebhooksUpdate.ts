 ...
 */
export function extensionsWebhooksUpdate(
  client: SquadcastSDKCore,
  request: operations.WebhooksUpdateWebhookRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.WebhooksUpdateWebhookResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.WebhooksUpdateWebhookRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["V3.Extensions.Webhooks.Webhook"], {
    explode: true,
  });

  const pathParams = {
    eventWebhookID: encodeSimple("eventWebhookID", payload.eventWebhookID, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/v3/extensions/event-webhooks/{eventWebhookID}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.WebhooksUpdateWebhookResponse$inboundSchema),
