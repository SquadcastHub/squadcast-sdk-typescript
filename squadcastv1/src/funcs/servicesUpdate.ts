...
export function servicesUpdate(
  ...
  Result<
    operations.ServicesUpdateServiceResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ServicesUpdateServiceRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["V3.Services.UpdateServiceRequest"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/v3/services/{serviceID}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ServicesUpdateServiceResponse$inboundSchema),
