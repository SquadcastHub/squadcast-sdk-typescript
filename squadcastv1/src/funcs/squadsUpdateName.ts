...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.SquadsUpdateSquadNameRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["V4.Squads.UpdateSquadNameRequest"], {
    explode: true,
  });

  const pathParams = {
    squadID: encodeSimple("squadID", payload.squadID, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/v4/squads/{squadID}/name")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.SquadsUpdateSquadNameResponse$inboundSchema),
    M.jsonErr(
