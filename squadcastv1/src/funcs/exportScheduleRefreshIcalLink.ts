 ...
 */
export function exportScheduleRefreshIcalLink(
  client: SquadcastSDKCore,
  request: operations.ExportRefreshScheduleIcalLinkRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.ExportRefreshScheduleIcalLinkResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ExportRefreshScheduleIcalLinkRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/v4/schedules/{scheduleID}/ical-link")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ExportRefreshScheduleIcalLinkResponse$inboundSchema),
    M.jsonErr(
